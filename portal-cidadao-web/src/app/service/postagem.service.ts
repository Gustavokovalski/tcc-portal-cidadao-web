...
export class PostagemService extends BaseService {
  ...
  public async listarTodos(
    ...
    confiabilidade = '',
    polling = false
  ): Promise<IBaseModel<IPostagemModel[]>> {
    let params = new HttpParams();
    if (bairro && bairro != '') params = params.append('bairro', bairro);
    ...
    if (mes && mes > 0) params = params.append('mes', mes);
  ...
  public async PostagensAbertasPorMes(
    mes: string
  ): Promise<IBaseModel<IPostagemModel[]>> {
    return this.httpClient
      .get<IBaseModel<IPostagemModel[]>>(
        `${this.apiBaseUrl}/postagem/mes/${mes}`
      )
      .toPromise();
  }
  public async resolverPostagem(
    id: number,
    resolvido: boolean
  ): Promise<IBaseModel<IPostagemModel>> {
    return this.httpClient
      .put<IBaseModel<IPostagemModel>>(
        `${this.apiBaseUrl}/postagem/${id}/${resolvido}`,
        id
      )
      .toPromise();
  }

  public async buscarMidiaPostagem(
    nomeArquivo: string
  ): Promise<IArquivoModel> {
    return this.httpClient
  ...
  public async listarCategorias(): Promise<IBaseModel<ICategoriaModel[]>> {
    return this.httpClient
      .get<IBaseModel<ICategoriaModel[]>>(
        `${this.apiBaseUrl}/postagem/categorias`
      )
      .toPromise();
  }

  public async excluirPostagem(
    id: number,
    excluir: boolean
  ): Promise<IBaseModel<IPostagemModel>> {
    return this.httpClient
      .put<IBaseModel<IPostagemModel>>(
        `${this.apiBaseUrl}/postagem/${id}/${excluir}`,
        id
      )
      .toPromise();
  }
  public async listarSubcategorias(): Promise<IBaseModel<IEnumModel[]>> {
    return this.httpClient
      .get<IBaseModel<IEnumModel[]>>(
        `${this.apiBaseUrl}/postagem/subcategorias`
      )
      .toPromise();
  }
